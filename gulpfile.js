...
const cssMap = () => {
  return gulp.src('source/sass/style-map.scss')
      .pipe(plumber())
      .pipe(sourcemap.init())
      .pipe(sass())
      .pipe(postcss([autoprefixer({
...
const syncServer = () => {
  ...
  gulp.watch('source/sass/**/*.{scss,sass}', gulp.series(css, cssMap));
...
const start = gulp.series(clean, svgo, copy, css, cssMap, sprite, js, pugToHtml, syncServer);

const build = gulp.series(clean, svgo, copy, css, cssMap, sprite, js, pugToHtml, optimizeImages);
